...
function today(){const all=S.day?.sites||[],need=all.filter(x=>Number(x.pending||0)>0),done=all.filter(x=>!Number(x.pending||0)&&Number(x.expected||0)>0);$('mainView').innerHTML='<div class="sectionHead"><div><h2>يحتاج انتباهك اليوم</h2><div class="sub">المواقع الناقصة أولاً.</div></div><div class="pill '+(need.length?'bad':'ok')+'">'+need.length+' موقع</div></div><div class="siteGrid">'+(need.length?need.map(siteCard).join(''):'<div class="card empty"><b>لا توجد مواقع ناقصة</b></div>')+'</div><div class="sectionHead"><div><h2>المواقع المكتملة</h2><div class="sub">لا تُحمّل إلا عند الطلب.</div></div><button id="toggleDone" class="btn ghost">عرض '+done.length+'</button></div><div id="doneSites" class="siteGrid hidden"></div>';wireSites();$('toggleDone').onclick=()=>{const b=$('doneSites'),show=b.classList.contains('hidden');if(show&&!b.dataset.loaded){b.innerHTML=done.map(siteCard).join('');b.dataset.loaded='1';wireSites()}b.classList.toggle('hidden');$('toggleDone').textContent=b.classList.contains('hidden')?'عرض '+done.length:'إخفاء'}}
...
function siteDetail(){const g=S.siteData;if(!g)return render();const r=g.employees||[],p=r.filter(x=>!x.status).length;$('mainView').innerHTML='<div class="sectionHead"><button id="backSites" class="btn ghost">← رجوع</button><div style="flex:1"><h2>'+esc(g.site_name||'')+'</h2><div class="sub">'+esc(g.client_name||'')+(g.project_name?' · '+esc(g.project_name):'')+' · '+work()+'</div></div><div class="guardStatus '+(p?'bad':'ok')+'">'+(r.length-p)+'/'+r.length+'</div></div><div class="card viewCard"><div class="guardRows">'+r.map(guardRow).join('')+'</div></div><div class="stickyAction"><div class="sub"><b>'+p+'</b> بدون تحضير · حددي الموجودين فعلياً.</div><button id="bulkPresent" class="btn primary" '+(p?'':'disabled')+'>تأكيد المحددين حاضر</button></div>';$('backSites').onclick=()=>{S.site=null;S.siteData=null;render()};wireGuards();$('bulkPresent').onclick=bulkPresent}
function guardRow(e){const p=!e.status,c=p?'pending':EX.has(e.status)?'exception':'';return '<div class="guardRow '+c+'"><div class="guardMain"><div style="display:flex;gap:8px;align-items:flex-start">'+(p?'<input class="check guardCheck" type="checkbox" value="'+e.assignment_id+'">':'')+'<div><div class="guardName">'+esc(e.full_name)+'</div><div class="guardMeta">'+esc(e.employee_ref||'بدون رقم وظيفي')+' · '+esc(e.shift_code||'وردية غير محددة')+'</div></div></div><button class="guardStatus '+(!e.status?'warn':e.status==='P'?'ok':'bad')+' editDay" data-a="'+e.assignment_id+'">'+(p?'لم يُحضّر':esc(STATUS[e.status]||e.status))+'</button></div>'+(p?'<div class="quickActions"><button class="quickBtn present qStatus" data-a="'+e.assignment_id+'" data-s="P">حاضر</button><button class="quickBtn off qStatus" data-a="'+e.assignment_id+'" data-s="OFF">راحة</button><button class="quickBtn absent qStatus" data-a="'+e.assignment_id+'" data-s="A">غياب</button><button class="quickBtn qMore" data-a="'+e.assignment_id+'">المزيد…</button></div>':'')+'</div>'}
...
async function gaps(){$('mainView').innerHTML='<div class="card empty" style="min-height:150px"><span class="loading"></span></div>';try{S.issues=await fast('issues',{period:period(),date:work()},10000);drawGaps()}catch(e){$('mainView').innerHTML='<div class="card empty">'+esc(e.message)+'</div>';toast(e.message,true)}}
function drawGaps(){const x=S.issues||{},c=x.counts||{},sec=(t,n,it)=>'<div class="card gapCard"><h3>'+esc(t)+'</h3><div class="count">'+Number(n||0)+'</div><div class="gapItems">'+it.slice(0,15).join('')+'</div></div>';const p=(x.pending_sites||[]).map(r=>'<div class="gapItem">'+esc(r.site_name||'بدون موقع')+' · '+Number(r.pending||0)+'</div>'),m=(x.missing_site_rows||[]).map(r=>'<div class="gapItem">'+esc(r.full_name||'')+'</div>'),u=(x.unmatched_rows||[]).map(r=>'<div class="gapItem">'+esc(r.full_name||'')+'</div>');$('mainView').innerHTML='<div class="sectionHead"><div><h2>النواقص والاستثناءات</h2><div class="sub">تُجلب عند فتح الصفحة فقط.</div></div></div><div class="gapList">'+sec('بدون تحضير',c.pending_today,p)+sec('تحتاج مطابقة موظف',c.unmatched,u)+sec('بدون موقع',c.missing_site,m)+sec('تعارضات محتملة',c.overlaps,[])+'</div>'}
