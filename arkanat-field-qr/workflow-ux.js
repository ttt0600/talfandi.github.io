...
function syncScan(){
  ...
  const nid=document.getElementById('nid'),phone=document.getElementById('phone'),action=document.querySelector('input[name="arkShiftAction"]:checked'),assignment=document.querySelector('input[name="arkAssignmentClass"]:checked');const nidOk=/^[12][0-9]{9}$/.test(String(nid&&nid.value||'').trim());const pval=String(phone&&phone.value||'').replace(/[\s-]/g,'');const phoneOk=/^(?:\+?966|00966|0)?5\d{8}$/.test(pval)||pval.length>=9;const ready=nidOk&&phoneOk&&!!action&&!!assignment;
  setBar(ready?'ready':'warn','الخطوة 1 من 3',ready?'البيانات جاهزة. التالي تحديد الموقع.':'أكمل الهوية والجوال واختر نوع العملية ونوع التكليف.','متابعة وإثبات التواجد',send,ready?'بعد الموقع ستظهر صورة الإثبات.':'لن يتم الإرسال قبل اكتمال البيانات.',ready);
}
