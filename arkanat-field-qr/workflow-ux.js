(()=>{
'use strict';
if(window.__ARK_FIELD_WORKFLOW_UX_V3)return;
...
function setBar(stage,kicker,title,buttonText,target,sub='',enabled=true){
  ...
  if(key!==lastKey){
    b.innerHTML='<div class="ark-wf-kicker">'+esc(kicker)+'</div><div class="ark-wf-title">'+esc(title)+'</div>'+(sub?'<div class="ark-wf-sub">'+esc(sub)+'</div>':'')+(buttonText?'<button id="arkWorkflowPrimary" class="ark-wf-btn" type="button" '+(enabled?'':'disabled')+'>'+esc(buttonText)+'</button>':'');
    lastKey=key;
  }
  const p=b.querySelector('#arkWorkflowPrimary');
  if(p){p.disabled=!enabled;p.onclick=()=>{if(!enabled||!target)return;try{target.click()}catch(_){}}}
...
function syncScan(){
  ...
  if(!f||!send){removeBar();return}
  if(!visible(f)){
    markMirrored(null);
    const st=text(state);
    if(/تم حفظ المسحة على الجهاز/.test(st)){setBar('warn','التسجيل غير مكتمل','تم حفظ البيانات محلياً بانتظار الاتصال.','',null,'لا تغلق الصفحة حتى يكتمل الإثبات.');return}
    if(/تعذر|خطأ|فشل/.test(st)){setBar('error','يلزم إجراء','لم تكتمل العملية. راجع الرسالة الظاهرة.','',null,'أعد المحاولة قبل إغلاق الصفحة.');return}
    if(/تم تسجيل التواجد|المسحة مسجلة مسبقاً/.test(st)){setBar('busy','الخطوة التالية','جارٍ تجهيز صورة الإثبات…','',null,'لا تغلق الصفحة الآن.');return}
    removeBar();return;
  }
  const busy=!!send.disabled||/جارٍ/.test(send.textContent||'')||/جارٍ/.test(text(gps));
  if(busy){setBar('busy','الخطوة 2 من 3','جارٍ تحديد الموقع وتسجيل البيانات…','',null,'لا تغلق الصفحة.');return}
  ...
  const nidOk=/^[12][0-9]{9}$/.test(String(nid&&nid.value||'').trim());
  const pval=String(phone&&phone.value||'').replace(/[\s-]/g,'');
  const phoneOk=/^(?:\+?966|00966|0)?5\d{8}$/.test(pval)||pval.length>=9;
  const ready=nidOk&&phoneOk&&!!action;
...
function boot(){
  ...
  try{new MutationObserver(schedule).observe(root,{childList:true,subtree:true,characterData:true,attributes:true,attributeFilter:['class','disabled','checked']})}catch(_){}
  document.addEventListener('input',schedule,true);document.addEventListener('change',schedule,true);document.addEventListener('focusin',schedule,true);
