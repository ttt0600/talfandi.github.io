(()=>{
'use strict';
if(window.__ARK_DIRECT_BOOTSTRAP_V536)return;
...
function bootGuard(){
  ...
  load('./shift-attendance.js','directShift536','__ARK_FIELD_SHIFT_ATTENDANCE_V2',false);
  load('./mobile-ux.js','directMobile536','__ARK_FIELD_MOBILE_UX_V4',false);
  later(()=>load('./photo-evidence.js','directPhoto536','__ARK_FIELD_PHOTO_EVIDENCE_V2',false),180);
  later(()=>load('./workflow-ux.js','directWorkflow536','__ARK_FIELD_WORKFLOW_UX_V4',false),320);
  later(()=>load('./photo-evidence-ux.js','directPhotoUx536','__ARK_FIELD_PHOTO_UX_V2',false),700);
}
function boot(){isolateRoute();if(activeScan())bootGuard();else if(!opsUiPresent()&&stored(MODE_KEY)!=='ops'){}else load('./reprint-core.js','fieldReprintCore536',null,false);refreshWorkerLater()}
isolateRoute();
