const CACHE='arkanat-field-qr-v5-32-20260916';
...
async function cacheFirstCurrent(request,key){const c=await currentCache();const hit=await c.match(key||keyFor(new URL(request.url)));if(hit)return hit;try{const r=await fetch(request,{cache:'no-store'});if(r.ok)await c.put(key||keyFor(new URL(request.url)),r.clone());return r}catch(_){return new Response('Offline',{status:503,statusText:'Offline'})}}
