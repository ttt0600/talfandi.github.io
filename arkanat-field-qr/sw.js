const CACHE='arkanat-field-qr-v5-26-20260916';
const CORE=['./','./index.html','./reprint.js','./photo-evidence.js','./photo-evidence-ux.js','./workflow-ux.js','./mobile-ux.js','./route-guard.js','./shift-attendance.js'];
...
async function injectBoot(response,url){
  ...
  if(!text.includes('id="fieldMobileUxScript"'))boot+=MOBILE_UX_BOOT;
...
async function freshShell(request,url){
  try{
    const shell=new URL('./index.html',self.registration.scope);shell.searchParams.set('__ark_sw','526');
  ...
  if(!hadPrior){
    try{
      ...
      await Promise.all(clients.map(async c=>{
        try{const u=new URL(c.url);if(u.origin!==self.location.origin||!u.pathname.startsWith(new URL(self.registration.scope).pathname))return;if(u.searchParams.get('__ark_first_boot')==='526')return;u.searchParams.set('__ark_first_boot','526');await c.navigate(u.href)}catch(_){}
      }));
    }catch(_){}
  }
})())});
