...
function clearRequired(){try{sessionStorage.removeItem(REQUIRED_KEY)}catch(_){} requiredEventId='';window.removeEventListener('beforeunload',beforeUnloadGuard)}
function beforeUnloadGuard(e){if(!requiredEventId)return;e.preventDefault();e.returnValue='';return''}
function armRequired(eventId,result){
  ...
  ensureStateObserver();showMandatoryReceipt();
}
...
function ensureStateObserver(){
  ...
  stateObserver=new MutationObserver(()=>{
    ...
    if(requiredEventId){showMandatoryReceipt();setTimeout(()=>renderPhotoStep(requiredEventId,true),0);return}
...
async function uploadRow(row){
  ...
  const z=await r.json().catch(()=>({}));if(!r.ok||!z.ok){const e=new Error(z.message||'تعذر رفع صورة الإثبات.');e.status=r.status;e.permanent=r.status>=400&&r.status<500&&r.status!==408&&r.status!==429;throw e}return z
}
...
function makePhotoShell(state,eventId){
  const old=document.getElementById('photoEvidenceBox');if(old&&old.dataset.eventId===eventId)return old;if(old)old.remove();
  const box=document.createElement('div');box.id='photoEvidenceBox';box.dataset.eventId=eventId;box.className='note';box.style.cssText='margin-top:16px;border:2px solid #1d6b4a;background:#eef7f2;scroll-margin-top:18px';
...
function renderPhotoStep(eventId,force=false){
  if(!hasToken()||!eventId||(!force&&eventId===lastPromptedEvent))return;lastPromptedEvent=eventId;let tries=0;
  const mount=()=>{
    ...
    const box=makePhotoShell(state,eventId),input=box.querySelector('#fieldPhotoInput'),take=box.querySelector('#fieldPhotoTake'),out=box.querySelector('#fieldPhotoState');let meta=null,metaPromise=null,processing=false;
    const loadMeta=(forceMeta=false)=>{if(meta&&!forceMeta)return Promise.resolve(meta);if(metaPromise&&!forceMeta)return metaPromise;metaPromise=apiJson({action:'meta',event_id:eventId,checkpoint_token:getToken()}).then(z=>{meta=z;metaPromise=null;if(z.already_uploaded){completeRegistration('صورة الإثبات محفوظة ومربوطة بالمسحة.');return z}if(!processing&&out&&document.body.contains(out))out.innerHTML='<span class="ok">الكاميرا جاهزة. التقط صورة الموقع لإكمال التسجيل.</span>';return z}).catch(e=>{metaPromise=null;if(!processing&&out&&document.body.contains(out))out.innerHTML='<span class="warn">سيعاد ربط بيانات المسحة تلقائياً عند التقاط الصورة.</span>';throw e});return metaPromise};
    loadMeta().catch(()=>{});take.onclick=()=>input.click();
    const processFile=async(file)=>{
      if(processing||!file)return;if(!freshEnough(file)){out.innerHTML='<p class="bad">الصورة المختارة تبدو قديمة. التقط صورة جديدة للموقع الآن.</p>';input.value='';take.disabled=false;return}
      const captured=new Date().toISOString();processing=true;take.disabled=true;out.innerHTML='<div class="spin"></div><div class="center">جارٍ تجهيز صورة الإثبات وربطها بالمسحة…</div>';
      let currentMeta;try{currentMeta=meta||await loadMeta(true)}catch(e){processing=false;take.disabled=false;out.innerHTML='<p class="warn">تعذر ربط بيانات المسحة مؤقتاً. لا تغلق الصفحة قبل إكمال الصورة.</p><div class="actions"><button id="fieldPhotoMetaRetry" type="button">إعادة المحاولة دون إعادة التصوير</button><button id="fieldPhotoNewCapture" type="button" class="soft">إعادة التصوير</button></div>';const retry=out.querySelector('#fieldPhotoMetaRetry'),again=out.querySelector('#fieldPhotoNewCapture');retry.onclick=()=>processFile(file);again.onclick=()=>{input.value='';out.innerHTML='';input.click()};return}
      if(currentMeta.already_uploaded){processing=false;completeRegistration('صورة الإثبات محفوظة ومربوطة بالمسحة.');return}
      try{
        const made=await compose(file,currentMeta,captured),u=URL.createObjectURL(made.blob);out.innerHTML='<img id="fieldPhotoPreview" alt="معاينة صورة الإثبات" style="width:100%;max-height:420px;object-fit:contain;border-radius:12px;margin-top:12px;background:#111"><div class="sub" style="margin-top:8px">الحجم بعد الضغط: '+Math.max(1,Math.round(made.blob.size/1024))+' كيلوبايت</div><div class="actions"><button id="fieldPhotoApprove" type="button">اعتماد وحفظ الصورة وإكمال التسجيل</button><button id="fieldPhotoRetake" type="button" class="soft">إعادة التصوير</button></div>';out.querySelector('#fieldPhotoPreview').src=u;
        ...
        out.querySelector('#fieldPhotoApprove').onclick=async()=>{const approve=out.querySelector('#fieldPhotoApprove'),retake=out.querySelector('#fieldPhotoRetake');approve.disabled=true;retake.disabled=true;out.insertAdjacentHTML('beforeend','<div id="fieldPhotoUploading"><div class="spin"></div><div class="center">جارٍ حفظ الصورة وإكمال التسجيل…</div></div>');const row={event_id:eventId,checkpoint_token:getToken(),blob:made.blob,width_px:made.width,height_px:made.height,captured_client_ts:captured,queued_at:Date.now()};try{await uploadRow(row);URL.revokeObjectURL(u);processing=false;completeRegistration('تم حفظ صورة الإثبات وربطها بالمسحة.')}catch(e){if(e&&e.permanent){URL.revokeObjectURL(u);processing=false;out.innerHTML='<p class="bad">'+h(e.message||'تعذر قبول الصورة.')+'</p><p class="warn">التسجيل ما زال غير مكتمل حتى حفظ صورة صالحة.</p><div class="actions"><button id="fieldPhotoRetryCapture" type="button">إعادة التصوير</button></div>';const rb=out.querySelector('#fieldPhotoRetryCapture');rb.onclick=()=>{input.value='';take.disabled=false;out.innerHTML='';input.click()};return}let queued=false;try{await dbPut(row);queued=true}catch(_){}URL.revokeObjectURL(u);processing=false;if(queued)completeRegistration('تم التقاط صورة الإثبات وحفظها على الجهاز، وستُرفع تلقائياً عند توفر الاتصال.',true);else out.innerHTML='<p class="bad">تعذر حفظ الصورة حالياً.</p><p class="warn">التسجيل غير مكتمل؛ أعد المحاولة قبل إغلاق الصفحة.</p>'}};
      }catch(e){processing=false;out.innerHTML='<p class="bad">'+h(e.message||'تعذر تجهيز الصورة.')+'</p><p class="warn">التسجيل غير مكتمل حتى حفظ الصورة.</p>';take.disabled=false}
    };
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'){setTimeout(flushPhotoQueue,700);if(requiredEventId){showMandatoryReceipt();setTimeout(()=>renderPhotoStep(requiredEventId,true),30)}}});
})();